...
*** Tangle on Save
Auto-tangle whenever the =.org= file is saved:
=(add-hook 'after-save-hook #'org-babel-tangle t t)=

*** Keep Tangled File in Sync
Add =:comments link= so tangled files contain links back to the source =.org= block.
This lets you edit the tangled file directly and propagate changes back with =org-babel-detangle=:
=#+property: header-args:emacs-lisp :tangle yes :comments link=

** Execute All Blocks
Run every source block in the current buffer:
=(org-babel-execute-buffer)=

** Caching
=:cache yes= in a block header skips re-evaluation if inputs haven't changed:
