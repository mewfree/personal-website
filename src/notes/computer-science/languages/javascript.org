...
TypeScript types flow directly from server to client — no REST schemas, no GraphQL codegen.
Built on React Query.

Use when: full-stack TypeScript (Next.js, SvelteKit...). Not ideal for public/multi-language APIs.
...
- =.query()= → read (GET semantics)
- =.mutation()= → write (POST/PUT/DELETE semantics)
