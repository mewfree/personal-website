...
* Decision Theory
** Loss Function
A function that maps an event or values to a real number representing some "cost".
Typically, the difference between estimated values and true values.
To optimize is to seek to minimize a loss function.
